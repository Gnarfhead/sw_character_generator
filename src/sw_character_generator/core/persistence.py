...
def _safe_json_dump(obj: Any, path: str) -> None:
    """Schreibe obj (in JSON-serialisierbarem Format) sicher nach path.
    Fehler werden gefangen und auf stderr geloggt.
    """
    try:
        with open(path, "w", encoding="utf-8") as fh:
            json.dump(obj, fh, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"Failed to save JSON to {path}: {e}", file=sys.stderr)

def _safe_json_load(path: str) -> Any:
    """Lade JSON von path. Gibt das geladene Objekt zurück oder None bei Fehlern."""
    try:
        with open(path, "r", encoding="utf-8") as fh:
            return json.load(fh)
